...
const Terms: React.FC = () => {
  ...
  if (!modalRoot) {
    console.error("Modal root (id='modal-root') not found in index.html");
    return null;
  }
  ...
  const modalContent = (
    <div className="modal" onClick={handleClose}>
      <div className="modal__content" onClick={(e) => e.stopPropagation()}>
        <button className="modal__close" onClick={handleClose} aria-label="Stäng modal">✕</button>
        {showModal === 'policy' && (
          <>
    ...
    <article className="terms">
      <ul className="terms__list">
        <li className="terms__list__item">
          <button onClick={() => setShowModal('policy')}>Integritetspolicy</button>
        </li>
        <li className="terms__list__item">
          <button onClick={() => setShowModal('terms')}>Användarvillkor</button>
        </li>
      </ul>
