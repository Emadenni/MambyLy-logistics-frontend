...
const TO_EMAIL = "info@mambylysolutions.se"; // opzionale metterlo in env
...
const sendOrderConfirmation = async () => {
  ...
  const extras_list = selected_extras.length > 0 ? selected_extras.join(", ") : "Inga";
  ...
  // Pagina extra a pagamento
  const paid_extra_pages_raw = localStorage.getItem("paid_extra_pages");
  let paid_extra_pages = "0";
  try {
    const parsedPages = JSON.parse(paid_extra_pages_raw || "[]");
    paid_extra_pages = Array.isArray(parsedPages) && parsedPages.length > 0
      ? parsedPages.join(", ")
      : "Inga";
  } catch {
    paid_extra_pages = paid_extra_pages_raw || "0";
  }
